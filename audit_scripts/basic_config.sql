SET SQLFORMAT CSV

set termout on
prompt basic_config.sql
set termout off

set termout on
prompt --audit_results/gv_database.csv
set termout off
spool audit_results/gv_database.csv
select * from gv$database;
spool off

set termout on
prompt --audit_results/cdb_services.csv
set termout off
spool audit_results/cdb_services.csv
select * 
from cdb_services 
order by con_id, service_id;
spool off

set termout on
prompt --audit_results/diag_info.csv
set termout off
spool audit_results/diag_info.csv
SELECT * 
FROM gv$diag_info 
where name <> 'Default Trace File'
order by inst_id, name, con_id;
spool off

set termout on
prompt --audit_results/server_info.csv
set termout off
spool audit_results/server_info.csv
select STAT_NAME,to_char(VALUE) as VALUE ,COMMENTS from v$osstat where
stat_name IN ('NUM_CPUS','NUM_CPU_CORES','NUM_CPU_SOCKETS')
union
select STAT_NAME,VALUE/1024/1024/1024 || ' GB' ,COMMENTS from
v$osstat where stat_name IN ('PHYSICAL_MEMORY_BYTES');
spool off

set termout on
prompt --audit_results/gv_instance.csv
set termout off
spool audit_results/gv_instance.csv
select * from gv$instance order by inst_id;
spool off
 
set termout on
prompt --audit_results/gv_parameter.csv
set termout off
spool audit_results/gv_parameter.csv
select * from gv$parameter order by isdefault, name;
spool off

set termout on
prompt --audit_results/gv_spparameter.csv
set termout off
spool audit_results/gv_spparameter.csv
select * from gv$spparameter order by isspecified, name;
spool off

set termout on
prompt --audit_results/gv_system_parameter.csv
set termout off
spool audit_results/gv_system_parameter.csv
select * from gv$system_parameter order by isdefault, name, con_id;
spool off

set termout on
prompt --audit_results/parameter_diffs.csv
set termout off
spool audit_results/parameter_diffs.csv
SELECT p.name,
       i.instance_name AS sid,
       p.value AS current_value,
       sp.sid,
       sp.value AS spfile_value      
FROM   v$spparameter sp,
       v$parameter p,
       v$instance i
WHERE  sp.name   = p.name
AND    sp.value != p.value;
spool off

set termout on
prompt --audit_results/gv_license.csv
set termout off
spool audit_results/gv_license.csv
select * from gv$license;
spool off

set termout on
prompt --audit_results/gv_version.csv
set termout off
spool audit_results/gv_version.csv
select * from gv$version;
spool off

set termout on
prompt --audit_results/gv_option.csv
set termout off
spool audit_results/gv_option.csv
select * from gv$option order by inst_id, parameter;
spool off

set termout on
prompt --audit_results/database_properties.csv
set termout off
spool audit_results/database_properties.csv
select * from database_properties order by property_name;
spool off

set termout on
prompt --audit_results/nls_database_parameters.csv
set termout off
spool audit_results/nls_database_parameters.csv
SELECT * FROM nls_database_parameters ORDER BY 1;
spool off

set termout on
prompt --audit_results/nls_instance_parameters .csv
set termout off
spool audit_results/nls_instance_parameters .csv
SELECT * FROM nls_instance_parameters ORDER BY 1;
spool off

set termout on
prompt --audit_results/nls_session_parameters.csv
set termout off
spool audit_results/nls_session_parameters.csv
SELECT * FROM nls_session_parameters ORDER BY 1;
spool off

set termout on
prompt --audit_results/registry_history.csv
set termout off
spool audit_results/registry_history.csv
select * from sys.registry$history order by action_time;
spool off

set termout on
prompt --audit_results/cdb_registry.csv
set termout off
spool audit_results/cdb_registry.csv
select * from cdb_registry order by con_id, comp_id, comp_name;
spool off

set termout on
prompt --audit_results/cdb_registry_database.csv
set termout off
spool audit_results/cdb_registry_database.csv
select * from cdb_registry_database order by con_id, platform_id;
spool off

set termout on
prompt --audit_results/cdb_registry_dependencies.csv
set termout off
spool audit_results/cdb_registry_dependencies.csv
select * from cdb_registry_dependencies order by con_id, comp_id, namespace, req_comp_id;

set termout on
prompt --audit_results/cdb_registry_hierarchy.csv
set termout off
spool audit_results/cdb_registry_hierarchy.csv
select * from cdb_registry_hierarchy order by con_id, comp_id, namespace, version;
spool off

set termout on
prompt --audit_results/cdb_registry_log.csv
set termout off
spool audit_results/cdb_registry_log.csv
select * from cdb_registry_log order by optime desc;
spool off

set termout on
prompt --audit_results/cdb_registry_sqlpatch.csv
set termout off
spool audit_results/cdb_registry_sqlpatch.csv
select * from cdb_registry_sqlpatch order by con_id, patch_id;
spool off

set termout on
prompt --audit_results/gv_system_fix_control.csv
set termout off
spool audit_results/gv_system_fix_control.csv
SELECT * FROM   gv$system_fix_control order by inst_id, bugno;
spool off

set termout on
prompt --audit_results/cdb_feature_usage_statistics.csv
set termout off
spool audit_results/cdb_feature_usage_statistics.csv
select * from cdb_feature_usage_statistics order by con_id, last_usage_date desc nulls last,dbid, name;
spool off